...
class BTWindow(Window):
    ...
    def get_bt_window(self):
        ...
        # TODO: лучше стараться не использовать глобальные переменные, если того не требует проект, их сложно отслеживать.
        #  Если надо использовать их в других классах, которые инициализуются в текущем проекте, то передавай их как
        #  аргументы при инициализации. Тогда ты будешь точно уверен, что тот класс будет исплоьзовать нужное значение.
        #  Иначе можно случайно в других модулях переопределить эту переменную, и значение уже будет другое.
        #  Если их надо использовать в других модулях - то лучше тогда выносить их из класса как отдельную перменную/класс
